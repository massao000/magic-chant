...
class WhisperMic:
    def __init__(self,model="base",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=False,energy=300,pause=2,dynamic_energy=False,save_file=False, model_root="~/.cache/whisper",mic_index=None):
        ...
        self.audio_model = WhisperModel(model, device=device)
    ...
    def __preprocess(self, data):
        return torch.from_numpy(np.frombuffer(data, np.int16).flatten().astype(np.float32) / 32768.0).detach().numpy().copy()
    ...
    def __transcribe(self,data=None, realtime: bool = False) -> None:
        ...
        if self.english:
            ...
            segments, _ = self.audio_model.transcribe(audio_data, language='english')
        else:
        ...
        if not self.verbose:
            ...
            if predicted_text not in self.banned_results:
                self.result_queue.put_nowait(segments)
